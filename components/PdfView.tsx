...
export const PdfView: React.FC<Props> = ({ 
  ...
  const canDownload = user.role === 'ADMIN' || (user.isPremium && (user.subscriptionTier === 'YEARLY' || user.subscriptionTier === 'LIFETIME' || (user.subscriptionTier === 'MONTHLY' && user.subscriptionLevel === 'ULTRA')));
  ...
  const handlePdfClick = (type: 'FREE' | 'PREMIUM' | 'ULTRA') => {
      ...
      if (type === 'FREE') {
          ...
          // STRICT MODE SEPARATION
          if (syllabusMode === 'SCHOOL') {
             link = contentData?.schoolPdfLink || contentData?.premiumLink; // Fallback for School (Legacy)
